...
known_pacman_package() {
  case "$1" in
    git|stow|tmux|fzf|fd|ripgrep|jq|bat|eza|fastfetch|btop|htop|ncdu|git-delta|direnv|tldr|neovim|zsh|curl|bash|wget|nano|ca-certificates|shellcheck|atuin|networkmanager|tailscale|playerctl|pamixer|power-profiles-daemon|hypridle|iw|ghostty|alacritty|vivaldi|firefox|gsimplecal|zoxide|yazi|satty|socat|restic|age|msmtp|obsidian|kdeconnect|wl-clipboard|libnotify|pacman|base-devel) return 0 ;;
    *) return 1 ;;
  esac
}
